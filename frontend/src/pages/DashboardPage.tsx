...
import { getDeclarations } from '@/api/client'
...
type State =
  | { kind: 'loading' }
  | { kind: 'loaded'; declarations: DeclarationListItemDto[] }
  | { kind: 'error' }
...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    getDeclarations()
      .then((declarations) => {
        if (!cancelled) setState({ kind: 'loaded', declarations })
      })
      .catch(() => {
  ...
  const declarations = state.kind === 'loaded' ? state.declarations : []
  ...
  const summaryText =
    state.kind === 'loading'
      ? 'Loading…'
      : state.kind === 'error'
        ? 'Could not load declarations'
        : hasDeclarations
          ? `${declarations.length} declaration${declarations.length === 1 ? '' : 's'} · last generated ${formatDate(declarations[0].createdAt)}`
  ...
  return (
    <main className="mx-auto max-w-[1120px] px-7 pt-9 pb-20">
      ...
      {state.kind === 'loading' && (
        <div aria-busy="true" aria-label="Loading declarations" className="animate-pulse rounded-2xl bg-muted h-72" />
      )}

      {state.kind === 'error' && (
        <Card className="rounded-2xl px-10 py-14 text-center">
          <CardContent className="p-0">
            <p className="text-sm text-muted-foreground">Could not load your declarations. Please refresh the page.</p>
          </CardContent>
        </Card>
      )}

      {state.kind === 'loaded' && hasDeclarations && <DeclarationsTable declarations={declarations} />}

      {state.kind === 'loaded' && !hasDeclarations && (
        <Card className="rounded-2xl px-10 py-14 text-center">
